...
export default function XOAnimation({ onComplete, onProgress }: XOAnimationProps) {
  ...
  useEffect(() => {
    ...
    // alpha:false = always opaque, never transparent, never white
    const ctx = canvas.getContext("2d", { alpha: false })!;
    const BG  = "#020916";

    // Light blue + white small bubbles — like reference video
    const COLORS = [
    ...
    function buildParticles() {
      ...
      type P = {
        ox: number; oy: number;   // origin on image
        x:  number; y:  number;   // current pos
        tx: number; ty: number;   // explode target
        r:  number;               // radius (small bubbles)
        color: string;
        alpha: number;
        ed: number;               // explode delay
        speed: number;
      };

      const pts: P[] = [];
      const STEP = 4; // dense sampling
      const scaleX = IW / OW;
      const scaleY = IH / OH;
      ...
      for (let y = 0; y < OH; y += STEP) {
        for (let x = 0; x < OW; x += STEP) {
          const i = (y * OW + x) * 4;
          if (d[i + 3] < 60) continue; // skip transparent

          const ox = IX + x * scaleX;
          const oy = IY + y * scaleY;

          // Explode direction: outward from center + some randomness
          const cx = W / 2, cy = H / 2;
          const ddx = ox - cx, ddy = oy - cy;
          const dist = Math.sqrt(ddx*ddx + ddy*ddy) || 1;
          const speed = 1.8 + Math.random() * 2.5;

          // Small bubble size like reference video
    ...
    // Timing
    const T_HOLD    = 1.8;   // XO image shows, holds
    const T_BREAK   = 2.4;   // particles start breaking off
    const T_DONE_P  = 4.8;   // particles fully gone
    const T_FADE    = 3.8;   // wrapper starts fading (hero reveals while particles still flying)
    const T_DONE    = 5.2;
    ...
    // Image opacity tracks separately so it fades as particles fly off
    let imgOpacity = 0;
    ...
    function frame(now: number) {
      ...
      // ── XO image: fade in, then fade out as particles break off ──
      if (e < T_HOLD) {
        imgOpacity = eOut(clamp(e / T_HOLD, 0, 1));
      } else if (e < T_BREAK) {
        imgOpacity = 1;
      } else if (e < T_BREAK + 0.8) {
        // Image fades out quickly as particles take over
      ...
      if (imgOpacity > 0) {
        ...
        ctx.drawImage(img, IX, IY, IW, IH);
        ctx.restore();
        // Actual sharp image on top
        ctx.globalAlpha = imgOpacity;
        ctx.drawImage(img, IX, IY, IW, IH);
        ctx.globalAlpha = 1;
      }

      // ── Build particles at break point ──
      ...
      if (e >= T_DONE && !done) {
        ...
        // One final dark frame paint before handing off — kills any white flash
        ctx.fillStyle = BG;
        ctx.fillRect(0, 0, W, H);
